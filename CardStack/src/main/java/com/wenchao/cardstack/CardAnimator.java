...
import static com.wenchao.cardstack.CardUtils.DIRECTION_BOTTOM_RIGHT;
import static com.wenchao.cardstack.CardUtils.DIRECTION_TOP_LEFT;
import static com.wenchao.cardstack.CardUtils.DIRECTION_TOP_RIGHT;
...
public class CardAnimator {
    ...
    private RelativeLayout.LayoutParams mLastParams;

    public CardAnimator(ArrayList<View> viewCollection, View lastView, int backgroundColor, int margin) {
    ...
    public void initLayout() {
        ...
        LayoutParams params = cloneParams(baseLayout);
        mLastView.setLayoutParams(params);
//        RelativeLayout.LayoutParams copy = new RelativeLayout.LayoutParams(500, 500);
//        mLastView.setLayoutParams(copy);
        scale(mLastView, -(size - 3) * 5, mGravity);

        int margin = 2 * mStackMargin;
        move(mLastView, mGravity == TOP ? -margin : margin, 0);

        RelativeLayout.LayoutParams paramsCopy =
                cloneParams((RelativeLayout.LayoutParams) mLastView.getLayoutParams());
        mLastParams = paramsCopy;
        setupRemotes();
    }
    ...
    public void drag(MotionEvent e1, MotionEvent e2, float distanceX,
        ...
        if (x_diff > 0) {
            for (View v : mCardCollection) {
                int index = mCardCollection.indexOf(v);
                if (index == 3 || index == 2) {
                    LayoutParams l = CardUtils.scaleFrom(v, mLayoutsMap.get(v), -(int) (Math.abs(x_diff) * 0.045), mGravity);
                    CardUtils.moveFrom(v, l, 0, -(int) (Math.abs(x_diff) * index * 0.05), mGravity);
                }
            }
